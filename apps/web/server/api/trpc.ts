...
/**
 * Bearer-token primitives live in `@/lib/api/bearer-token` so the `/api/v1`
 * mount reuses this exact mechanism rather than growing a parallel one (see
 * that file's header). Behaviour here is unchanged — the five function bodies
 * moved unchanged apart from the added `export`, and every call site below is
 * untouched.
 */
import {
