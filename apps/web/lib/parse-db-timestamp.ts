 ...
 * stored value is the UTC rendering of the instant (contract
 * `docs/research/api-platform/plans/005-phase0-contract.md` §2 N3). PostgREST
 * (supabase-js) returns them zone-less, e.g. `"2026-07-15T18:30:00"`, and
 * `new Date(value)` parses such strings as *local* time — so in any zone east
 * of UTC the instant lands hours early and late-evening rounds render on the
 * previous date. This helper appends `Z` before parsing so the value is read
 * back as the UTC instant it is.
 *
