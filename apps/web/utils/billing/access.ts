...
/**
 * Determines if a user has unlimited access based on JWT billing claims.
 *
 * This is a more restrictive check than hasPremiumAccess().
 * Unlimited access is required for dashboard and statistics pages.
 *
 * Access Rules:
 * 1. Deny immediately for problematic statuses: past_due, incomplete, paused
 * 2. For "canceled" status: allow access until current_period_end (grace period)
 * 3. For all statuses: deny if current_period_end is expired (defense-in-depth)
 * 4. Otherwise: grant access only if plan is unlimited or lifetime
 *
 * @param billing - Billing claims from JWT (app_metadata.billing)
 * @returns true if user has unlimited access, false otherwise
 */
export function hasUnlimitedAccess(billing: BillingClaims | null | undefined): boolean {
  if (!billing) return false;
  ...
  if (status === "canceled") {
    if (cancel_at_period_end && current_period_end) {
      const nowSeconds = Date.now() / 1000;
      const isExpired = nowSeconds > current_period_end + EXPIRY_LEEWAY_SECONDS;
      return !isExpired && (plan === "unlimited" || plan === "lifetime");
    }
    return false; // Canceled without grace period
  }
  ...
  // Rule 4: Default - check if plan is unlimited tier (not premium)
