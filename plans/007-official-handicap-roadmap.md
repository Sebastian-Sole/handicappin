...
- **Issue**: https://github.com/Sebastian-Sole/handicappin/issues/143

## Why this matters

The owner's stated goals (2026-07-12): the app serves golfers worldwide; **medium-term**, its handicap must become official/recognized (some courses require an official handicap index to book/play); **long-term**, position Handicappin to pitch Norges Golfforbund (NGF) on replacing GolfBox as Norway's administrative system. The owner's original framing was "get USGA approved". Research (July 2026, summarized below — treat it as the source material to distill, not something to re-research) established that **no jurisdiction licenses software vendors to issue a Handicap Index**; the realistic paths run through licensed clubs/associations and their APIs. That strategy spans months of owner-side actions (applications, agreements) that must not live only in a chat log. This plan turns the research into a durable decision document with concrete next actions, so every future session (and the owner) works from the same map.
...
4. **Route B — Norway & GB&I: companion + partnership** — iGolf/OpenPlay/NGF facts; NGF Union-database monitoring; England Golf ISV enquiry.
   Include a subsection **"B-long: the NGF/GolfBox endgame"** — the owner's long-term goal is to pitch NGF on Handicappin replacing GolfBox. Frame it honestly: GolfBox is a full club-administration platform (memberships, tee-time booking, tournaments, national database, club finance), not just handicap computation — replacing it means becoming a club-facing admin system, a different product tier from a consumer tracker. The 2025 NGF decision to build an **NGF-owned Union database** "to enable choosing other administrative systems in the future" is precisely the door this goal needs; the staged path is: consumer credibility + flawless WHS engine → official integrations elsewhere (Route A proves legitimacy) → pilot club-facing features with 1–2 friendly Norwegian clubs → formal NGF conversation when the Union database opens. State clearly that steps toward this are relationship and product-surface investments, not a near-term engineering project.
...
7. **Owner action checklist** — the exact next moves: ① contact 1–2 target AGAs about Type-3 club licensing (requirements, cost, timeline); ② contact USGA about the GPA vendor program (exact name, eligibility, API access levels); ③ email England Golf about ISV/CONGU licence terms for a consumer app; ④ monitor NGF's Union-database rollout and request an intro conversation — this is also the first move of the B-long endgame, so treat the NGF relationship as strategic, not transactional; ⑤ decide the peer-attestation question; ⑥ identify 1–2 friendly Norwegian clubs as future pilot candidates for club-facing features (B-long groundwork). Each with "blocked on owner" marker, mirroring BACKLOG's style.
