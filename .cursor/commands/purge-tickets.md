...
This command performs backlog refinement/grooming by reviewing tickets in the `.cursor/tickets/` directory and determining which tickets should be deleted, archived, or kept active.
...
For each ticket in `.cursor/tickets/`, evaluate:
...
```bash
# Ticket remains in .cursor/tickets/
# No changes needed
```
...
mkdir -p .cursor/tickets/archive
mv .cursor/tickets/XXXX-ticket-name.md .cursor/tickets/archive/
```
...
```bash
# Remove completely
rm .cursor/tickets/XXXX-ticket-name.md
```
...
ls -1 .cursor/tickets/*.md 2>/dev/null | grep -v archive || echo "No tickets found"
```
...
3. **Assess Documentation Value**
   ...
   - Check for associated experience files in `.cursor/experiences/`
...
- Check against `.cursor/rules/` for current project boundaries
