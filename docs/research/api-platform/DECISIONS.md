...
## Sign-off: post-build contract amendments (owner, 2026-08-08) — 6 DECISIONS LOCKED

Elicited after Waves 2–3 merged (PRs #194–#197, all five day-one endpoints live on `main`). These close the six pending owner items the wave-3 handoff surfaced; each resolves a silence or defect in `plans/005-phase0-contract.md` that verification found, and each is recorded as an in-place amendment there tagged `[Amended 2026-08-08 — D1x]`. Wave 4 (T13.5 OpenAPI + CI parity) was blocked on D10 and is now dispatchable.

**D10 — `GET /v1/health` is the sole unauthenticated `/v1` endpoint.** §6's "all `/api/v1` endpoints require Bearer auth" prose is amended to name this one exception. Rationale is recorded in the route header (`app/api/v1/health/route.ts`): the ingress canary cannot hold a token; an authenticated probe cannot reproduce the cookie-less edge-block failure the canary exists to detect; the body is a fixed literal with nothing to protect. In the OpenAPI spec the operation carries an empty security override (`security: []`); every other operation inherits the global Bearer requirement. **Irreversibility: none — adding auth to health later would break the canary by design, so this is intended to be permanent.**

**D11 — Mint `rounds:read`; reads gate on read-OR-write.** `V1_SCOPES` gains `rounds:read`, the `custom_access_token_hook` stamps OAuth tokens `rounds:read rounds:write` (was `rounds:write` only — a read-only OAuth client was structurally impossible), and scoped read endpoints gate on `hasScope(rounds:read) || hasScope(rounds:write)`. `POST /v1/rounds` stays `rounds:write`-only. For every existing token this is a pure relaxation — a `rounds:write` token passes every gate it passed before. Requires a migration (hook change), which applies to prod on merge per `.github/workflows/migrate.yml`. When Supabase ships per-client scopes, a read-only client becomes a configuration change, not a contract change.

**D12 — The read-gate split is ratified as shipped, and is now contractual.** Catalog reads (`GET /v1/courses`, `GET /v1/tees`) require **authentication only** — no scope check, no entitlement (`plan_required`) gate. Rounds reads (`GET /v1/rounds`) keep the entitlement gate and are scope-gated per D11. §4 makes *adding* a gate to a shipped read breaking, so the catalog's openness is now a commitment, not an accident: catalog data is not user data, and a token that can write rounds must be able to resolve the catalog it writes against. The prior defect was the contract's silence, not either behavior.

**D13 — `ScoreHoleMismatchError` → 422 `validation_failed`.** Added to §1's domain-error mapping table with field-level code `score_hole_mismatch` in `errors[]` (field-level codes are append-only by design). Previously unmapped: the catch-all sent a client's bad cross-section hole reference to 500 `internal_error` + a Sentry page, where tRPC returns 400 for the same input. The tRPC adapter keeps BAD_REQUEST; `/v1` is stricter-and-typed, same pattern as `CourseResolutionError`.

**D14 — Catalog-read `course_not_found` (422) ratified as shipped.** `GET /v1/tees` with an unknown `courseId` returns **422 `course_not_found`**, the same code and status as a write referencing an unknown tee — the client's remedy (search-resolve reads) is identical, and one code for one condition beats two codes keyed on which endpoint noticed. `not_found` (404) remains reserved for resource-instance lookups. §1's registry row now says so explicitly. Zero code change; the shipped behavior already argued this in `app/api/v1/tees/route.ts`'s header.

**D15 — Pre-auth traffic gets its own rate-limit family.** Pre-auth / invalid-token requests stay keyed `ip:{ip}` but move to a **dedicated fail-closed limiter** with its own Redis prefix (`ratelimit:public-api:preauth`) and its own env-tunable budget (`RATE_LIMIT_PREAUTH_PER_MIN`, default 300/min, mechanism frozen / number is an ops value per D6's pattern). Motivation: the amplification fix meters pre-auth traffic per IP, and fitbull's requests all originate from Convex's few egress IPs — under the shared budget a burst of token-expiry 401s could cap the entire fitbull fleet. Authenticated per-route budgets never draw from the IP family and vice versa. **Gates fitbull go-live.**
