...
- **Compared fields** — every client-controlled field that determines the stored round (from the v1 submission schema derived from `apps/web/types/scorecard-input.ts`): `teeId`, `teeTime`, `nineHoleSection` (absent ≡ 18-hole), `notes`, and the per-hole `scores` array: `strokes`, `putts`, `penaltyStrokes`, `fairwayHit`, and shot-detail fields if the v1 schema admits them.
...
**Implementation note (005 build):** these three normalizations are **merge-blocking test cases** — retry-without-putts (N1), retry-of-a-back-nine-round (N2), and retry-of-the-same-instant-with-a-different-timezone-offset (N3) must each return the **200 replay**, not a 409.
