...
**Date:** 2026-07-29 (rev. 2 — adversarial-review fixes) · **Status:** FROZEN pending the three owner sign-off items at the end · **Workstream:** W4 Phase 0 (`plans/005-w4-v1-contract-and-handlers.md` §Phase 0; Checkpoint A per `plans/000-INDEX.md`).
...
- `public.get_connected_entitlement()` — a SECURITY DEFINER entitlement accessor, **required** before any `/v1` route can evaluate `plan_required` (§1; without it every OAuth request false-positives).
...
| `plan_required` | 403 | Account exists but has **not completed plan selection**. Determined via `get_connected_entitlement()` (below) — never by a failed `profile` read. Remedy differs by principal class (below). A real error (DECISIONS §Billing gate). |
...
RETURNS TABLE (
  ...
  rounds_used          integer   -- COUNT(round) for auth.uid() WHERE quarantined = false
)
LANGUAGE sql SECURITY DEFINER SET search_path = public STABLE
AS $$ ... WHERE p.id = auth.uid() ... $$;   -- row filter hard-coded to auth.uid(), as in get_connected_profile()
...
- **The decision procedure** (`externalId` present):
  1. **Lookup first:** `SELECT ... FROM round WHERE "userId" = $1 AND "externalId" = $2`.
  2. **Row found** → compare bodies per the rules below → rule **2** (200 replay) or rule **3** (409 `idempotency_conflict`). The natural key is irrelevant here, whether or not it also collided.
  3. **No row** → rule **5**: natural-key lookup → 409 `duplicate_round`.
- **The natural-key lookup that produces `existingRoundId`** (rules 4 and 5) — the handler must do this itself, because `DuplicateRoundError` carries only `key: "natural-key" | "external-id"` and **never a round id** (`errors.ts:74-84` on branch `api-platform/003-bundled-migration`): `SELECT id FROM round WHERE "userId" = $1 AND "teeId" = $2 AND "teeTime" = $3 AND ("nine_hole_section" = $4 OR ($4 IS NULL AND "nine_hole_section" IS NULL))`. It reuses **N1**'s absent ≡ null rule for `nine_hole_section` (an 18-hole round submits it absent and stores NULL, and the constraint is `NULLS NOT DISTINCT`, so the predicate must match NULL to NULL — a plain `=` silently finds nothing and would degrade the 409 into a 500) and **N3**'s UTC-instant canonicalization for `teeTime` (the same value the constraint compared). If the lookup finds no row, the violation was not one of these two keys and the error passes through unmapped.
- **Ordering:** both lookups run **after** the failed insert's transaction has rolled back. Inside an aborted transaction every subsequent statement raises `25P02 in_failed_sql_transaction`, so the lookup must be issued on a clean connection/transaction, not appended to the doomed one.
...
**Scope of the guarantee — `/v1` only, and deliberately so.** The quarantine promise holds for rounds written **through `POST /v1/rounds`**. The same OAuth token can `INSERT` directly into `round` via PostgREST — `20260728091000_oauth_client_rls_deny.sql:31-36` leaves round/score `INSERT`/`UPDATE`/`SELECT` open by design (write-only-by-default posture) and denies only `DELETE` — and such a row lands **`quarantined = false`, i.e. active**, bypassing the limit check entirely. This is the RLS insert side door already recorded as a **shipping gate** (billing-and-metering condition 1) and as an open item in `003-notes.md` (which additionally notes the permissive round-UPDATE policy lets a user clear `quarantined` on their own row). The contract therefore states the guarantee as a property of the `/v1` path, not as an absolute property of the account, and closing the side door with a DB-level guard (trigger or restrictive policy) remains a precondition for the token-bearing consumer going live.
...
- **One answer for a scope-less token.** `403 forbidden` fires **only** when a token *carries* a `scope` claim that does not permit the operation. A token with **no** `scope` claim is a first-party token, no scope check applies to it, and it is full-capability on its own user exactly as on tRPC. There is no third behavior: a missing `scope` claim is never itself a denial.
...
- `get_connected_entitlement()`'s existence and its exact four returned fields (§1) — the corpus requires that `plan_required` be determinable but specifies no mechanism, and the shipped code provides none.
- `POST /v1/profile/provision` being **first-party-only**, with the OAuth remedy being user-side plan selection (§1). The corpus calls the endpoint an "auth-decision-independent fallback" without noticing the RLS insert denial.
...
- **Deciding rules by lookup rather than by constraint name**, and the ordering requirement that lookups run after rollback.
...
- That a **missing** `scope` claim is never itself a denial.
