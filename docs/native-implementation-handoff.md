...
5. Visual parity evidence exists for every ported route: `compare-screen.sh` captures committed-adjacent (artifacts dir is gitignored — store a summary table in the implementation log). If `ANTHROPIC_API_KEY` is present in the environment, the vision-judge harness verdict is PASS (or PENDING_HUMAN_SIGN_OFF with captures ready) for each route; if absent, captures + a note suffice — do not block on the key.
...
- **Vision-judge harness** (`harness:run <screen>`): sim capture vs web reference, 2-vote Claude quorum (`claude-sonnet-4-6` default, `ANTHROPIC_MODEL` override; no API key → degrades to human sign-off, don't block). Caps: 8 iterations/screen, no-progress halt, verdict cache keyed on inputs+rubric+model. **The judge prompt is uncalibrated — calibrate on your first ported screen** (run, read the judgment, tighten the rubric, repeat once).
