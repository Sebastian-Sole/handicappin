import type { Hole, Score, Tee } from "./shared-schemas.ts";
import {
...
/**
 * Mirror of packages/handicap-core/src/timeline.ts.
 *
 * The Deno edge runtime can't import from the workspace package, so this
 * file is a hand-mirrored copy for edge functions. `scripts/check-handicap-sync.mjs`
 * enforces that this stays byte-for-byte in sync (modulo import specifiers
 * and the documented `getRelevantRounds`/`RoundLike` omission). Do NOT edit
 * this file independently of packages/handicap-core/src/timeline.ts.
 */
 ...
 * (never reimplemented) from `packages/handicap-core/src/timeline.ts`,
 * and `scripts/check-handicap-sync.mjs` enforces that the mirror stays
