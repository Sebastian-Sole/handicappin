...
export const teeSchema = z
  .object({
    ...
    // (mirrors apps/web/types/scorecard-input.ts): approved par-3 courses
    // rate far below 40, and one such round in a user's history must not
    // wedge their handicap recalculation forever.
    courseRating18: z
      .number()
    ...
    courseRatingFront9: z
      .number()
      .min(9, "Front 9 rating must be at least 9")
    ...
    courseRatingBack9: z
      .number()
      .min(9, "Back 9 rating must be at least 9")
