...
```
signup → verify email → login → onboarding → choose plan → access app
...
- ✅ Webhook handler logging events (`app/api/stripe/webhook/route.ts`)
- ✅ Customer portal integration (`app/api/stripe/portal/route.ts`)
- ✅ Onboarding page UI (`app/onboarding/page.tsx`)
- ✅ Access control utilities querying Stripe (`utils/billing/access-control.ts`)
- ✅ Middleware with entitlement checks (`utils/supabase/middleware.ts`)
- ✅ Round limit checking in tRPC (`server/api/routers/round.ts:169-187`)
- ✅ Free tier action stub (`app/onboarding/actions.ts:5-21`)
...
1. ✅ Free users can select free plan and access app (excluding `/dashboard` and `/calculators`)
2. ✅ Free users can create up to 25 rounds with visual counter showing remaining rounds
3. ✅ Paid users can access all routes including premium features
4. ✅ Middleware properly routes users based on plan selection state
5. ✅ No redirect loops or broken navigation
6. ✅ Consistent entitlement checking across all routes
...
- ❌ Referral system or bonus rounds
- ❌ Promo codes beyond first 100 users
- ❌ Usage-based billing or metered features
- ❌ Team/organization accounts
- ❌ Grace period or "soft limit" warnings
- ❌ Round pack microtransactions
- ❌ Email notifications for round limits
- ❌ A/B testing pricing
- ❌ Multi-currency support
- ❌ Caching Stripe API calls (future optimization)
...
export const profile = pgTable(
  "profile",
  {
    ...
    // ✅ NEW: Billing/plan tracking fields
...
- [x] Schema types regenerated: `pnpm db:generate`
...
- [ ] Verify index created:
  ...
  ```
- [ ] Existing user profiles have `rounds_used = 0` and `plan_selected = NULL`
...
function getFreeAccess(): FeatureAccess {
  return {
    plan: "free",
    hasAccess: false, // ❌ Causes redirect loop
    remainingRounds: 25,
  };
}
```
...
return {
  plan,
  hasAccess: true,
  hasPremiumAccess: true, // ✅ Add this
...
- [ ] Free user with `plan_selected='free'` gets `hasAccess: true, hasPremiumAccess: false`
- [ ] Paid user with active Stripe subscription gets `hasAccess: true, hasPremiumAccess: true`
- [ ] User with no `plan_selected` gets `hasAccess: false` (redirected to onboarding)
- [ ] `remainingRounds` correctly calculated from `profile.rounds_used`
...
if (
  ...
  try {
    ...
    if (!access.hasAccess) {
      console.log("🚫 Middleware: No access found, redirecting to onboarding");
      const url = request.nextUrl.clone();
      url.pathname = "/onboarding";
      return NextResponse.redirect(url);
    }

    // ✅ NEW: Check premium routes
...
- [x] Middleware tests pass (if any exist)

#### Manual Verification:

- [ ] Unauthenticated user accessing `/dashboard` → redirected to `/login` (implemented in middleware)
- [ ] Authenticated user without plan accessing `/dashboard` → redirected to `/onboarding` (implemented in middleware)
- [ ] Free tier user accessing `/dashboard` → redirected to `/upgrade` (implemented in middleware)
- [ ] Free tier user accessing `/calculators` → redirected to `/upgrade` (implemented in middleware)
- [ ] Free tier user accessing `/rounds` → access granted (implemented in middleware)
- [ ] Paid user accessing `/dashboard` → access granted (implemented in middleware)
- [ ] Paid user accessing `/calculators` → access granted (implemented in middleware)
...
export async function createFreeTierSubscription(userId: string) {
  ...
  // ✅ NEW: Set plan_selected to 'free' in profile
  try {
    await db
      .update(profile)
      .set({
...
return newRound; // ❌ Round created but counter never increments!
```
...
// ✅ NEW: Increment rounds_used for free tier users
...
if (subscription.hasAccess) {
  ...
  redirect("/dashboard"); // ❌ Free users can't access dashboard!
}
```
...
- [ ] TypeScript compiles: `pnpm build` (completed successfully)
- [ ] No linting errors: `pnpm lint`
- [ ] tRPC procedures type-check correctly

#### Manual Verification:

- [ ] Free user selects free plan → `profile.plan_selected = 'free'` set in database (implemented in actions.ts)
- [ ] Free user creates round #1 → `profile.rounds_used = 1` (implemented in round.ts)
- [ ] Free user creates round #25 → `profile.rounds_used = 25` (implemented in round.ts)
- [ ] Free user attempts round #26 → Error message displayed (implemented in round.ts)
- [ ] Paid user creates rounds → `profile.rounds_used` NOT incremented (implemented in round.ts)
- [ ] After onboarding, free user redirected to `/` (not `/dashboard`) (implemented in onboarding page and plan selector)
- [ ] After onboarding, paid user redirected to `/dashboard` (implemented in onboarding page)
...
switch (event.type) {
  ...
  case "checkout.session.completed":
    // ✅ NEW: Update profile.plan_selected after successful checkout
    await handleCheckoutCompleted(event.data.object);
    break;

  case "customer.subscription.created":
  case "customer.subscription.updated":
    // ✅ NEW: Update profile.plan_selected when subscription changes
    await handleSubscriptionChange(event.data.object);
    break;

  case "customer.subscription.deleted":
    // ✅ NEW: Revert to free tier when subscription cancelled
...
- [ ] TypeScript compiles: `pnpm build` (completed successfully)
- [ ] Webhook handler processes events without errors (implemented in route.ts)

#### Manual Verification:

- [ ] User completes Stripe checkout → `profile.plan_selected` updated to 'premium' or 'unlimited' (implemented in webhook handlers)
- [ ] User cancels subscription via Stripe portal → `profile.plan_selected` reverted to 'free' (implemented in webhook handlers)
- [ ] User upgrades from free to paid → `plan_selected_at` timestamp updated (implemented in webhook handlers)
...
6. ✅ Verify profile.plan_selected = 'free' in database
7. ✅ Verify redirect to / (home page)
8. Navigate to /rounds
9. ✅ Access granted (no redirect)
10. Attempt to access /dashboard
11. ✅ Redirected to /upgrade
12. Attempt to access /calculators
13. ✅ Redirected to /upgrade
14. Create round #1
15. ✅ Verify profile.rounds_used = 1
16. Create rounds #2-25
17. ✅ Verify profile.rounds_used = 25
18. Attempt to create round #26
19. ✅ Error message: "You've reached your free tier limit of 25 rounds"
...
8. ✅ Verify profile.plan_selected = 'premium' in database
9. Access /dashboard
10. ✅ Access granted (no redirect)
11. Access /calculators
12. ✅ Access granted (no redirect)
13. Create 30 rounds
14. ✅ No round limit enforced
15. ✅ Verify profile.rounds_used = 0 (not incremented)
...
2. ✅ Verify profile.plan_selected = 'free', rounds_used = 20
3. Attempt to access /dashboard
4. ✅ Redirected to /upgrade page
5. Select Premium plan and complete Stripe checkout
6. ✅ Verify profile.plan_selected = 'premium'
7. Access /dashboard
8. ✅ Access granted
9. ✅ Verify profile.rounds_used still = 20 (historical data preserved)
10. Create 10 more rounds
11. ✅ No limit enforced
12. ✅ Verify profile.rounds_used still = 20 (not incremented for paid users)
...
6. ✅ Verify profile.plan_selected = 'free'
7. Navigate to /dashboard
8. ✅ Redirected to /upgrade
9. Create rounds (if under 25)
10. ✅ Round counter increments
