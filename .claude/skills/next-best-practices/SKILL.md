...
description: "Fires when writing or reviewing Next.js code — file conventions, RSC boundaries, data patterns, async APIs, metadata, error handling, route handlers, image/font optimization, bundling."
...
- **Next.js 16 has breaking changes from training data.** Always read `node_modules/next/dist/docs/` before using unfamiliar APIs. Don't assume training data is correct.
- **`params` and `searchParams` are async in Next.js 15+.** Must `await` them in page/layout components.
- **Middleware was renamed to `proxy` in v16.** Check `references/file-conventions.md` for the current name.
- **React Compiler is enabled.** Don't add manual memoization (`useMemo`, `useCallback`, `React.memo`).
- **`react-dom/server` is blocked in Route Handlers.** Use alternative rendering approaches for server-side HTML generation (e.g., newsletter rendering).
