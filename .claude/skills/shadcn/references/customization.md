...
Add variables to the file at `tailwindCssFile` from `pnpm dlx shadcn@latest info` (typically `globals.css`). Never create a new CSS file for this.
...
export function ConfirmDialog({ title, description, onConfirm, children }) {
  return (
    <AlertDialog>
      <AlertDialogTrigger render={children}>{children}</AlertDialogTrigger>
      <AlertDialogContent>
        <AlertDialogHeader>
